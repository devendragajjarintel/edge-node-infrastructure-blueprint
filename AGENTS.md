...
- `profile-enclosure`: Orchestrate a full profiling session — chain `set-power-profile` → `set-thermal-profile` → `monitor-platform-power` → `generate-platform-stress` (apply → monitor → stress → summarize) with a single confirmation and a bounded `duration`, then emit one consolidated enclosure report (min/mean/max of PkgTmp/PkgWatt/GFXWatt plus a throttle/headroom verdict). Use to qualify whether an enclosure can sustain a chosen profile under load.
