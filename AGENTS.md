...
- `set-platform-power`: Set the local platform power envelope with `tools/power-tuning/set_power_profile.sh` — PkgWatt (PL1) from 5 W to the silicon cTDP (Level 2) in multiples of 5, with a configurable burst ratio and an optional independent SysWatt (psys) cap.
- `set-power-profile`: Apply a named platform power profile (LowPower 10 W, BalancedLow 15 W, BalancedHigh 20 W, Performance 25 W, MaxPerformance = platform max / cTDP Level 2) by PkgWatt budget with `tools/power-tuning/set_power_profile.sh` — PkgWatt (the package cap) is enforced on every platform; each profile has a default burst ratio (overridable), and on psys-capable silicon the SysWatt cap tracks the same PkgWatt budget or an explicit `--sysWatt` value when supplied.
