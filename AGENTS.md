...
- `set-platform-power`: Set the local platform power envelope with `tools/power-tuning/set_platform_power.sh` — PkgWatt (PL1) from 5 W to the silicon cTDP (Level 2) in multiples of 5, with a configurable burst ratio and an optional independent SysWatt (psys) cap.
- `set-power-profile`: Apply a named platform power profile (LowPower 10 W, BalancedLow 15 W, BalancedHigh 20 W, Performance 25 W, MaxPerformance 45 W) by SysWatt budget with `tools/power-tuning/set_power_profile.sh` — each profile has a default burst ratio (overridable), and PkgWatt is estimated as `SysWatt - uncoreWatt` (default 5 W) when the silicon has no psys (SysWatt) domain.
