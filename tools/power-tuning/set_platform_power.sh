...
#
# Usage:   sudo ./set_platform_power.sh [--pkgWatt W] [--sysWatt W] \
#                                      [--burstRatio R] [--pl1Tau S]
...
usage() {
	cat <<EOF
Usage: $0 [--pkgWatt W] [--sysWatt W] [--burstRatio R] [--pl1Tau S]
       $0 --help

Set an intel_lpmd power profile and enforce it with a hard RAPL power cap
(CPU + iGPU). The Config-TDP levels are read from the CPU's RAPL MSRs at runtime.
...
while [[ $# -gt 0 ]]; do
	case "$1" in
		-h|--help) usage; exit 0 ;;
		--pkgWatt|--sysWatt|--burstRatio|--pl1Tau)
			if [[ $# -lt 2 ]]; then
...
# --pkgWatt is optional; when omitted it defaults to the Nominal TDP read from
# the MSRs below. Validate the supplied values.
...
# Default target = Nominal TDP (the CPU's rated sustained power) when no <watts>
# argument was supplied on the command line.
if [[ -z "$WATTS" ]]; then
	WATTS="$NOMINAL_TDP"
	echo "No <watts> given; defaulting PL1 to Nominal TDP ${WATTS}W."
fi
...
cat > "$tmp_cfg" <<EOF
<?xml version="1.0"?>
<!-- Auto-generated by set_platform_power.sh for ~${WATTS}W. Pinned single state. -->
<Configuration>
